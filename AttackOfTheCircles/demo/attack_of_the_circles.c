...
const SDL_Color SDL_LIGHTER_RED = {255, 0, 70};
const int PTSIZE = 20;
...
void on_key(char key, KeyEventType type, double held_time, void* data){
    ...
    switch(key){
        ...
        case 'c':
            ...
            break;
        case '\r':
            if (scene_get_finished_title_screen(scene) == false){
              scene_set_finished_title_screen(scene, true);
              curr_level = 0;
...
void gen_title_text(void){
    write_font(FONT_REGULAR, 4 * PTSIZE, "ATTACK OF THE CIRCLES", SDL_LIGHTER_RED,
    (Vector){250, 130}, 500, 50);
    write_font(FONT_REGULAR, 2 * PTSIZE, "Press enter to continue", SDL_LIGHTER_RED,
    (Vector){350, 320}, 300, 30);
}
...
int main(int argc, const char* argv[]){
    ...
    while(!sdl_is_done()){
        ...
        if(curr_level != -1 && curr_level != 5){
          add_gui(scene, min_corn, max_corn, info->MAX_BULLETS);
        }
        regen_bullets(scene);
